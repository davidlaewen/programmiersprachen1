...
def typeCheck(e: Exp, gamma: Map[String,Type]) : (List[(Type,Type)],Type) = e match {
  ...
  case Fun(p,b) =>
    val fromType = freshTypeVar()
    val resBody = typeCheck(b,gamma+(p->fromType))
    (resBody._1, FunType(fromType,resBody._2))
  case App(f,a) =>
...
/** Full type checker */
def doTypeCheck(e: Exp)(gamma: Map[String,Type]): Type = {
  val (eqs,t) = typeCheck(e,gamma)
  unify(eqs)(t)
}
...
/** Task */
val e1 = Fun("x", "x")
// unify( List() )( FunType(TypeVar(X),TypeVar(X)) )
// ~~> FunType(TypeVar(X),TypeVar(X)))

val e2 = Fun("x", Add("x", 1))
// unify( List(TypeVar(X)->NumType(), ...) )( FunType(TypeVar(X),NumType()) )
// ~~> FunType(NumType(),NumType())

//This term is in an open context.
val e3: Exp = Add("x", 1)
val e3Gamma = Map("x" -> freshTypeVar)
// unify( List(TypeVar(X)->NumType(), ...) )( NumType() )
// ~~> NumType()

val apply: Exp = Fun("f", Fun("x", App("f", "x")))
// FunType(FunType(TypeVar(X),TypeVar(Y)), FunType(TypeVar(X), TypeVar(Y)))

val compose: Exp = Fun("f", Fun("g", Fun("x", App("g", App("f", "x")))))
// FunType(FunType(TypeVar(X),TypeVar(Y)),
//   FunType(FunType(TypeVar(Y),TypeVar(Z)),
//     FunType(TypeVar(X),TypeVar(Z))))
// (X => Y) => (Y => Z) => (X => Z)
