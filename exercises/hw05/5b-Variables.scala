...
def assign(id: String, e: Exp) : Exp = SetBox(id,e)
...
def eval(e: Exp, env: Env, s: Store) : (Value,Store) = e match {
  ...
  case App(f,a) => eval(f,env,s) match {
    case (ClosureV(Fun(p,b),cEnv),s1) => eval(a,env,s1) match {
      case (v,s2) =>
        val a = nextAddress
        eval(b, cEnv+(p -> AddressV(a)), s2+(a -> v))
    }
...
val test1 =
  App(Fun("x", wth("y", Num(0),
    Seq(If0(OpenBox("x"), assign("y", 4), assign("y", 2)),
      Mul(OpenBox("y"), OpenBox("y"))))), 5)
assert(eval(test1,Map(),Map())._1 == NumV(4))

val test2 =
  App(Fun("x", wth("y", Num(0),
    Seq(If0(OpenBox("x"), assign("y", 4), assign("y", 2)),
      Mul(OpenBox("y"), OpenBox("y"))))), 0)
assert(eval(test2,Map(),Map())._1 == NumV(16))
