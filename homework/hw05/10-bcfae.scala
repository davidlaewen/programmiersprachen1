...
/**
should give result ``1`` in a proper implementation.
...
In order to demonstrate the actual nature of mutation, we will not use mutation in our meta-language to implement mutation
...
Note that changing the value of a in the example is not a violation of static scope.  Scoping only says where an identifier is bound;
...
/**
The other, which we call _store_, tracks dynamic changes.
...
def eval(e: Exp, env: Env, s: Store) : (Value, Store) = e match {
  ...
  => eval(cond, env, s) match {
    ...
    /* An alternative that enforces runtime type-correctness of
...
def gc(env: Env, store:Store) : Store = {
  ...
  def allAddrInEnv(env: Env) : Set[Address] =
    env.values.map(allAddrInVal).fold(Set.empty)(_ union _)
...
assert(eval(bcFaeProgram1, Map.empty, Map.empty)._1 == NumV(3))
assert(eval(bcFaeProgram2, Map.empty, Map.empty)._1 == NumV(2))
