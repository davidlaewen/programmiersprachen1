...
Call-by-name can be explained very succinctly in the substitution-based interpreter: Instead of evaluating the argument "a" in the
...
hence becomes a mapping from symbols to thunks. Note that environments and thunks are hence mutually recursive. In Scala, we can
therefore not use type definitions of the form
