...
In the last lecture we have seen how we can give commonly occurring (sub)expressions a name via the "with" construct. Often, however,
...
def eval(funs: Funs, e: Exp) : Int = e match {
  ...
  case Call(f,args) => {
     ...
     if (fd.args.size != vargs.size) sys.error("number of parameters in call to " + f + " does not match")
...
__Deﬁnition (Static Scope)__: In a language with static scope, the scope of an identifier’s binding is a syntactically delimited region.
A typical region would be the body of a function or other binding construct.
 
__Deﬁnition (Dynamic Scope)__: In a language with dynamic scope, the scope of an identifier’s binding is the entire remainder of the
